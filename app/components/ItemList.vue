<template>
  <div>
    ...
    <div
      ...
      class="text-center py-16 rounded-2xl border border-dashed border-rule-2 bg-paper-2"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="mx-auto h-12 w-12 text-rule-2 mb-4"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        stroke-width="1.5"
      ...
      <p class="text-xs text-muted max-w-[16rem] mx-auto">
        Add your first item above to start tracking what expires next
      </p>
    </div>

    <ul v-else class="space-y-2 list-none p-0 m-0">
      <li v-for="item in sortedItems" :key="item.id">
        <ItemCard :item="item" @delete="$emit('delete', $event)" />
      </li>
    </ul>
  </div>
</template>
