<template>
  <div class="flex items-center justify-between gap-3 rounded-xl border border-rule bg-paper px-4 py-3 shadow-sm transition-colors duration-[var(--dur-short)] hover:border-rule-2">
    <div class="flex items-center gap-3 min-w-0">
      <StatusBadge :status="getExpiryStatus(item.expiryDate)" />
      <div class="min-w-0">
        <p class="text-sm font-semibold text-ink truncate">{{ item.name }}</p>
        <p class="text-xs text-muted mt-0.5">{{ formatDate(item.expiryDate) }}<span class="text-muted/70">· {{ daysLabel }}</span></p>
      </div>
    </div>
    <button class="relative flex-shrink-0 rounded-lg p-2 text-muted hover:text-danger hover:bg-danger-bg focus-visible:ring-2 focus-visible:ring-danger/30 focus-visible:outline-none active:translate-y-px transition-colors duration-[var(--dur-short)] [&::before]:absolute [&::before]:inset-[-4px] [&::before]:content-[''] touch-manipulation"
      style="min-height: 44px; min-width: 44px" :aria-label="`Delete ${item.name}`" @click="$emit('delete', item.id)">
